...
def main(isexternal,howMuchSplit,isMicro,tupaType,isMotabo,MotaboFileName,DataFileName,ClassFileName,sampleNameFile,variableNameFile,scale_type,iteration,survivalRate,V_rankingAlgorithm, nComponent):
    ...
    startNum, endNum = genStartEndNum2.gaussian_algorithm(int(classNum), classList,sampleList,  V_rankingAlgorithm,nComponent)
    ...
    for k in range(ITERATION):
        ...
        return_idx, sample_taining, sample_test, class_training, class_test = newScore.setNumber(int(classNum), classList, sampleList, startNum, endNum, howMuchSplit,k, class_trans_dict, scale_type, V_rankingAlgorithm, nComponent)
    ...
    if isexternal:
        if classNum == 2:
           gen_roc_graph(scale_training_sample[:,valid_idx],classList,scaled_external[:,valid_idx],external_class,"output/rocExternal/roc_external.png", 'ROC External ')
        else:
            mul_roc_graph(classNum,class_num_label,classList,external_class,scale_training_sample[:,valid_idx],scaled_external[:,valid_idx],ROC_COLOR,"output/rocExternal/ROC External ",isMicro,'ROC External B', class_trans_dict)
    ...
    if classNum == 2:
        gen_roc_graph(scale_training_sample,classList,scale_training_sample,classList,'output/rocTrainNoFS/rocTrainNoFS.png','ROC Training, No Feature Selection ')
    else:
        mul_roc_graph(classNum, class_num_label, classList, classList, scale_training_sample,
                      scale_training_sample, ROC_COLOR, 'output/rocTrainNoFS/rocTrainNoFS',isMicro,'ROC Training, No Feature Selection ', class_trans_dict)
    ...
    # generate predict ROC
    if classNum == 2:
        gen_roc_graph(scale_training_sample,classList,scaled_external,external_class,'output/rocValiNoFS/rocValiNoFS.png', 'ROC Validation, No Feature Selection ')
    else:
        mul_roc_graph(classNum, class_num_label, classList, external_class, scale_training_sample,
                      scaled_external, ROC_COLOR, 'output/rocValiNoFS/rocValiNoFS',isMicro,'ROC Validation, No Feature Selection ', class_trans_dict)
    ...
    # generate predict ROC
    if classNum == 2:
        gen_roc_graph(scale_training_sample[:,valid_idx],classList,scale_training_sample[:,valid_idx],classList,'output/rocTrainFS/rocTrainFS.png','ROC Training, With Feature Selection ')
    else:
        mul_roc_graph(classNum, class_num_label, classList, classList, scale_training_sample[:,valid_idx],
                      scale_training_sample[:, valid_idx], ROC_COLOR, 'output/rocTrainFS/rocTrainFS',isMicro, 'ROC Training, With Feature Selection ', class_trans_dict)
    ...
    # generate predict ROC
    if classNum == 2:
        gen_roc_graph(scale_training_sample[:,valid_idx],classList,scaled_external[:,valid_idx],external_class,'output/rocValiFS/rocValiFS.png', 'ROC Validation, With Feature Selection ' )
    else:
        mul_roc_graph(classNum, class_num_label, classList, external_class, scale_training_sample[:, valid_idx],
                      scaled_external[:, valid_idx], ROC_COLOR,'output/rocValiFS/rocValiFS',isMicro, 'ROC Validation, With Feature Selection ', class_trans_dict)
...
if __name__ == "__main__":
    ...
    main(isexternal,rateSplit,isMicro,tupaType,isMotabo,motaboFileName,dataFileName,classFileName,sampleNameFileName,variableNameFileName,scaleType,howManyIteration,survivalrate,V_rankingAlgorithm,nComponent)